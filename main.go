...
#cgo LDFLAGS: -L../external/KEM-NIZKPoP/frodo-zkpop/frodo976 -lfrodo
*/
import "C"
...
func testFrodoKEM(N int) {
	fmt.Println("Testing Frodo976KEM...")
	...
	fmt.Printf("KeyPair976:  Total = %.4f s, Avg = %.6f s/op, StdDev = %.6f\n", totalKeypair, kpAvg, stddev(kpTimes, kpAvg))
	fmt.Printf("Encaps976:   Total = %.4f s, Avg = %.6f s/op, StdDev = %.6f\n", totalEncaps, encAvg, stddev(encTimes, encAvg))
	fmt.Printf("Decaps976:   Total = %.4f s, Avg = %.6f s/op, StdDev = %.6f\n", totalDecaps, decAvg, stddev(decTimes, decAvg))
}

func testFrodoKEMNIZKPoP(N int) {
	fmt.Println("Testing Frodo976KEM-NIZKPoP...")
	...
	for i := 0; i < N; i++ {
		...
		if err != nil {
			log.Fatalf("Frodo976 Keypair NIZKPoP failed on iteration %d: %v", i, err)
		}
		...
		if pk == nil || len(pk) == 0 || zkpopProof == nil || len(zkpopProof) == 0 {
			log.Fatalf("Frodo976 Invalid input to verification on iteration %d", i)
		}

		start = time.Now()
		valid := zkpop.VerifyFrodo976ZKPop(pk, zkpopProof)
		if !valid {
			log.Fatalf("Frodo976 Verification failed on iteration %d", i)
		}
	...
	fmt.Printf("KeyGen976:   Total = %.4f s, Avg = %.6f s/op, StdDev = %.6f\n", totalKeygen, keygenAvg, stddev(keygenTimes, keygenAvg))
	fmt.Printf("Verify976:   Total = %.4f s, Avg = %.6f s/op, StdDev = %.6f\n", totalVerify, verifyAvg, stddev(verifyTimes, verifyAvg))
	fmt.Printf("Combined976: Total = %.4f s, Avg = %.6f s/step (KeyGen+Verify)\n", total, (total)/float64(N*2))
}

func main() {
	N := 1000
	fmt.Printf("Testing %d iterations for Frodo976...\n\n", N)

	testFrodoKEM(N)
	fmt.Println()
	testFrodoKEMNIZKPoP(N)

	fmt.Println("\nEnd of Frodo976 testing.")
}
